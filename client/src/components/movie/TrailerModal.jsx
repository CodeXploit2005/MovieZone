...
const TrailerModal = ({ movieId, mediaType, onClose }) => {
  ...
  useEffect(() => {
    const fetchTrailer = async () => {
      try {
        ...
        // Fetch details which includes videos (backend now returns both local and English videos)
        let data = await movieService.getMovieDetails(movieId, lang, type);
        
        let videos = data.videos?.results || [];
        
        if (videos.length > 0) {
          // Priority: Trailer > Teaser > Clip > Featurette
          // Prefer trailers first
          const trailer = 
