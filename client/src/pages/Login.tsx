...
const Login = () => {
  ...
  const { login, t, formatMsg } = useAuth();
  ...
  const onSubmit = async (data: any) => {
    ...
    if (result.success) {
      toast.success(formatMsg(t('welcome_back')));
      // Check if user is admin to show gateway
      const storedUserStr = localStorage.getItem('moviezone_user');
      if (storedUserStr) {
      ...
      toast.error(result.message === 'Invalid credentials' ? formatMsg(t('invalid_credentials')) : formatMsg(result.message || ''));
    }
  };
