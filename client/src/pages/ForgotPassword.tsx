...
const ForgotPassword = () => {
  ...
  const handleSendOtp = async (e: React.FormEvent) => {
    ...
    try {
      const data = await authService.checkEmail(trimmedEmail);
      
      if (data.success) {
        setGeneratedOtp(data.otp);
        toast.success(formatMsg(t('otp_sent')));
        setStep(2);
      }
    } catch (error: any) {
      console.error('Forgot Password Error:', error);
      const errorMsg = formatMsg(error.response?.data?.message || t('email_not_found'));
  ...
  const handleVerifyOtp = (e: React.FormEvent) => {
    ...
    if (otp.trim() === generatedOtp) {
      ...
      toast.error(formatMsg(t('otp_invalid')));
    }
  };
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    const trimmedEmail = email.toLowerCase().trim();
    try {
      const data = await authService.resetPassword(trimmedEmail, newPassword);
      
      if (data.success) {
        toast.success(formatMsg(t('password_reset_success')));
        navigate('/login');
      }
    } catch (error: any) {
      toast.error(formatMsg(error.response?.data?.message || 'Error'));
    } finally {
      setLoading(false);
    }
  };
