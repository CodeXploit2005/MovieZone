...
import { 
  ...
  FiSlash, FiUserCheck, FiArrowRight, FiActivity, FiSend
} from 'react-icons/fi';
import { 
...
const AdminDashboard = () => {
  ...
  const [editingAdminMsg, setEditingAdminMsg] = useState(null); // { id, content }
  const [adminList, setAdminList] = useState([]);
  const [selectedReceiver, setSelectedReceiver] = useState('group'); // 'group' or admin object

  // Support Chat State
  const [supportConversations, setSupportConversations] = useState([]);
  const [selectedSupportUser, setSelectedSupportUser] = useState(null); // User object
  const [supportMessages, setSupportMessages] = useState([]);
  const [supportReply, setSupportReply] = useState('');
  const [supportLoading, setSupportLoading] = useState(false);
  const [editingSupportMsg, setEditingSupportMsg] = useState(null); // { id, content }

  // Quick View State
  const [activeQuickView, setActiveQuickView] = useState(null); // 'users', 'active', 'comments', 'movies'
  ...
  const fetchChatMessages = useCallback(async () => {
    try {
      ...
      const data = await adminService.getMessages(user.token, receiverId);
  ...
  const fetchSupportMessages = useCallback(async (userId) => {
    try {
  ...
  }, [selectedReceiver, activeTab, fetchChatMessages, fetchSupportConversations]);
  ...
  const handleUpdateSupportMsg = async (e) => {
    e.preventDefault();
  ...
  const handleDeleteSupportMsg = async (id) => {
  ...
  const handleSendChatMessage = async (e) => {
    ...
    try {
      const receiverId = selectedReceiver === 'group' ? 'group' : selectedReceiver._id;
      const data = await adminService.sendMessage(user.token, {
        content: newMessage,
        receiverId
      });
  ...
  const handleDeleteAdminMsg = async (id) => {
  ...
  const handleUpdateAdminMsg = async (e) => {
    e.preventDefault();
    if (!editingAdminMsg.content.trim()) return;
    try {
    ...
    try {
      ...
      setComments(comments.map(c => {
        ...
        if (editingComment.isReply && c._id === editingComment.parentId) {
          return {
            ...c,
            replies: c.replies.map(r => r._id === editingComment.id ? { ...r, content: editingComment.content } : r)
          };
        }
        return c;
      }));
  ...
  const handleAddMovie = async (e) => {
    ...
    try {
      ...
      const movieData = { ...movieForm, genres: genresArray };
  ...
  const handleEditMovieClick = useCallback((movie) => {
  ...
  const formatChartData = useMemo(() => {
    return stats.healthChart?.map(item => {
          ...
          <div className="lg:col-span-3 space-y-2">
            {tabs.map(tab => (
              <button
                ...
                className={`w-full flex items-center space-x-4 p-5 rounded-2xl transition-all font-black uppercase tracking-widest text-[10px] ${
                  activeTab === tab.id 
                ...
                >
                  ...
                  <div className={`w-72 border-r flex flex-col ${theme === 'dark' ? 'border-white/5 bg-white/[0.02]' : 'border-gray-100 bg-gray-50/50'}`}>
                    <div className="p-6 border-b border-white/5">
                      <h4 className="font-black uppercase tracking-widest text-[10px] text-neutral-500 mb-4">{t('admin_list')}</h4>
                      <div className="space-y-2">
                        <button 
                          onClick={() => setSelectedReceiver('group')}
                    ...
                    <div className="flex-grow overflow-y-auto p-4 space-y-2 custom-scrollbar">
                      {adminList.filter(a => a._id !== user._id).map(admin => (
                        <button 
                          key={admin._id}
                          onClick={() => setSelectedReceiver(admin)}
                          className={`w-full flex items-center gap-3 p-3 rounded-2xl transition-all ${
                    ...
                    <div className={`p-8 border-b flex items-center justify-between ${theme === 'dark' ? 'border-white/5 bg-white/5' : 'border-gray-100 bg-gray-50'}`}>
                      <div>
                        <h3 className="font-black uppercase tracking-widest text-sm flex items-center">
                          ...
                          {selectedReceiver === 'group' ? t('admin_chat_room') : `${t('chat_with')} ${selectedReceiver.username}`}
                        </h3>
                        <p className="text-[9px] text-neutral-500 font-bold uppercase mt-1">
                          {selectedReceiver === 'group' ? t('admin_chat_desc') : t('private_chat_desc')}
                        </p>
                      </div>
                      {selectedReceiver === 'group' && (
                        <div className="flex -space-x-2">
