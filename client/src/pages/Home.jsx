...
const Home = () => {
  ...
  // Pagination states for infinite scroll rows
  ...
  const sliderMovies = useMemo(() => [
    ...customBanners.map(b => ({
      id: b.movie?._id || b.link,
      title: b.title,
      overview: b.movie?.description || '',
      backdrop_path: b.image,
      poster_path: b.movie?.posterPath || b.image,
      vote_average: b.movie?.voteAverage || 0,
      release_date: b.movie?.releaseDate || '',
      isCustom: true
    })),
    ...trending.slice(0, 5),
    ...popular.slice(0, 2) // Fallback
  ].filter((v, i, a) => a.findIndex(t => (t.id === v.id)) === i), [customBanners, trending, popular]); // Unique
  ...
  return (
    <div className={`transition-colors duration-300 ${theme === 'dark' ? 'bg-[#050505]' : 'bg-[#f8f9fa]'} pb-20`}>
      ...
      <div className="container mx-auto px-4 md:px-10 mt-16">
        <div className="flex flex-col lg:flex-row gap-10">
          ...
          <div className="lg:w-3/4 space-y-12">
            ...
            <div className={`p-8 rounded-3xl border transition-colors ${theme === 'dark' ? 'bg-white/5 border-white/5' : 'bg-white border-gray-100 shadow-sm'}`}>
              <div className="flex items-center space-x-4 mb-8">
                <div className="w-1.5 h-8 bg-primary rounded-full shadow-[0_0_10px_rgba(229,9,20,0.5)]"></div>
                <h2 className={`text-2xl font-black uppercase ${theme === 'dark' ? 'text-white' : 'text-dark'}`}>{t('top_rated')}</h2>
              </div>
              ...
              <div className="mt-10 text-center">
                <Link to="/search" className="inline-block bg-primary text-white px-10 py-3 rounded-full font-black uppercase tracking-widest text-xs hover:bg-red-700 transition-all shadow-lg shadow-primary/20">
                  {t('explore_all')}
                </Link>
              </div>
            </div>
          ...
          <div className="lg:w-1/4 space-y-10">
            ...
            <div className={`p-6 rounded-3xl border transition-colors space-y-6 ${theme === 'dark' ? 'bg-white/5 border-white/5' : 'bg-white border-gray-100 shadow-lg'}`}>
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3 text-neutral-500">
                  <FiActivity size={18} />
                  <span className="text-[10px] font-black uppercase tracking-widest">{t('live_status')}</span>
                </div>
                <span className="text-primary font-black text-sm">2,451</span>
              </div>
              <div className={`h-[1px] w-full ${theme === 'dark' ? 'bg-white/5' : 'bg-gray-100'}`}></div>
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3 text-neutral-500">
                  <FiClock size={18} />
                  <span className="text-[10px] font-black uppercase tracking-widest">{t('database_stats')}</span>
                </div>
                <span className={`${theme === 'dark' ? 'text-white' : 'text-dark'} font-black text-sm`}>15,842</span>
              </div>
            </div>

            <div className={`p-8 rounded-3xl border transition-colors ${theme === 'dark' ? 'bg-white/5 border-white/5' : 'bg-white border-gray-100 shadow-lg'}`}>
              <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-neutral-500 mb-6">{t('popular_keywords')}</h3>
              <div className="flex flex-wrap gap-2">
                {['Hành Động', 'K-Drama', 'Marvel', 'Anime', 'Kinh Dị', 'Top IMDb'].map(tag => (
                  <Link key={tag} to={`/search?q=${tag}`} className={`text-[9px] font-black uppercase tracking-widest px-4 py-2 rounded-xl transition-all border ${theme === 'dark' ? 'text-neutral-400 bg-white/5 border-white/5 hover:bg-primary hover:text-white hover:border-primary' : 'text-neutral-600 bg-gray-50 border-gray-100 hover:bg-primary hover:text-white hover:border-primary'}`}>
                    {tag}
                  </Link>
                ))}
              </div>
