...
const ActivityHistory = () => {
  ...
  const handleDeleteHistory = async (movieId) => {
    ...
    try {
      ...
      setActivities(prev => ({
        ...
        replies: prev.replies.map(c => {
          ...
          return c;
        }).filter(c => c.replies.some(r => r.user?._id === user._id || r.user === user._id))
      }));
          ...
          >
            ...
            {activeTab === 'replies' && (
              <div className="space-y-4">
                ...
                ) : activities.replies.map((comment, i) => {
                  const userReplies = comment.replies?.filter(r => r.user?._id === user._id || r.user === user._id);
                  return (
                    <div 
                      key={i}
                      className={`p-8 rounded-[2.5rem] border transition-all ${
