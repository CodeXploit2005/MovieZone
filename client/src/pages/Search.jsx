...
const Search = () => {
  ...
  const fetchMovies = useCallback(async (searchQuery, pageNum, searchType, isLoadMore = false) => {
    setLoading(true);
    try {
      ...
      // TMDB API limits pagination to 500 pages
      const cappedTotalPages = data.total_pages > 500 ? 500 : data.total_pages;
  ...
  const renderPagination = () => {
    ...
    return (
      <div className="flex flex-wrap items-center justify-center gap-2 mt-16 mb-10">
        <button
        ...
        {startPage > 1 && (
          <>
            ...
            {startPage > 2 && <span className="text-neutral-500 font-bold px-1">...</span>}
          </>
        )}
        ...
        {endPage < totalPages && (
          <>
            {endPage < totalPages - 1 && <span className="text-neutral-500 font-bold px-1">...</span>}
            <button
      ...
      <div className="container mx-auto px-4 md:px-10">
        <div className="max-w-6xl mx-auto mb-16 space-y-8">
          <div className="flex-grow max-w-4xl mx-auto w-full relative group">
            <div className="absolute inset-y-0 left-0 pl-6 flex items-center pointer-events-none text-primary/50 group-focus-within:text-primary transition-colors">
              <FiSearch size={24} />
            </div>
            <input 
              type="text" 
              ...
              className={`w-full bg-white/5 border rounded-[2rem] py-6 pl-16 pr-16 text-xl outline-none transition-all duration-500 font-medium ${isFocused ? 'bg-white/10 border-primary shadow-[0_0_40px_rgba(229,9,20,0.2)]' : 'border-white/10'}`}
            />
            {query && (
              <button onClick={clearSearch} className="absolute inset-y-0 right-0 pr-6 flex items-center text-neutral-500 hover:text-white transition-colors">
                <FiX size={24} />
              </button>
            )}
          </div>
          ...
          <div className={`p-8 rounded-[2.5rem] border flex flex-wrap items-center gap-8 transition-all duration-300 ${theme === 'dark' ? 'bg-white/5 border-white/10 shadow-2xl backdrop-blur-md' : 'bg-white border-gray-100 shadow-xl'}`}>
            <div className="flex items-center space-x-3 text-primary border-r border-white/10 pr-6">
              <FiFilter size={20} />
              <span className="text-[11px] font-black uppercase tracking-[0.2em]">Bộ lọc:</span>
            </div>
            <select value={filterGenre} onChange={(e) => setFilterGenre(e.target.value)} className={`bg-transparent text-[11px] font-black uppercase outline-none cursor-pointer hover:text-primary transition-colors appearance-none ${theme === 'dark' ? 'text-neutral-400' : 'text-neutral-600'}`}>
            ...
            <select value={filterYear} onChange={(e) => setFilterYear(e.target.value)} className={`bg-transparent text-[11px] font-black uppercase outline-none cursor-pointer hover:text-primary transition-colors appearance-none ${theme === 'dark' ? 'text-neutral-400' : 'text-neutral-600'}`}>
            ...
            <div className="ml-auto flex items-center space-x-6 border-l border-white/10 pl-8">
          ...
          <>
            {movies.length > 0 && (
              <div className="mb-12 flex items-center justify-between">
                <h2 className={`text-2xl font-black uppercase tracking-tighter ${theme === 'dark' ? 'text-white' : 'text-dark'}`}>
                  {query ? (
                ...
                <div className="px-4 py-1 bg-primary/10 rounded-full text-primary text-[10px] font-black uppercase">{movies.length} Phim</div>
              </div>
            )}
            ...
            {movies.length === 0 && query && !loading && (
              <div className="text-center py-40 max-w-xl mx-auto space-y-6">
                <div className="w-24 h-24 bg-primary/10 rounded-full flex items-center justify-center mx-auto text-primary"><FiSearch size={40} /></div>
                <h3 className="text-2xl font-black uppercase tracking-tighter text-white">Không tìm thấy phim</h3>
                <p className="text-neutral-500 font-medium">Chúng tôi không tìm thấy kết quả nào phù hợp với "{query}". Vui lòng thử từ khóa khác hoặc sử dụng bộ lọc.</p>
              </div>
            )}
