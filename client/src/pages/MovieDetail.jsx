...
const MovieDetail = () => {
  ...
  const [replyingTo, setReplyTo] = useState(null); // commentId
  const [replyToUser, setReplyToUser] = useState(null); // username
  ...
  return (
    <div className={`transition-colors duration-300 ${theme === 'dark' ? 'bg-[#0a0a0a] text-white' : 'bg-[#f8f9fa] text-black'} pb-32`}>
      <AnimatePresence>
        {showVideo && trailer && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[200] bg-black/95 flex items-center justify-center p-4 md:p-10">
            <button onClick={() => setShowVideo(false)} className="absolute top-6 right-6 text-white hover:text-primary transition-all z-[210]"><FiX size={40} /></button>
      ...
      <div className="container mx-auto px-4 md:px-10 lg:px-20 -mt-64 relative z-10">
        <div className="flex flex-col lg:flex-row gap-16">
          <div className="lg:w-[350px] flex-shrink-0 space-y-8">
            ...
            <div className="grid grid-cols-2 gap-4">
              <button onClick={toggleFavorite} className={`py-4 rounded-2xl font-black uppercase tracking-widest text-[10px] transition-all flex items-center justify-center space-x-2 border ${isFavorite ? 'bg-white text-black border-white' : 'bg-white/5 text-neutral-400 border-white/10 hover:bg-white/10 hover:text-white'}`}>{isFavorite ? <FiCheck size={18} /> : <FiPlus size={18} />}<span>{isFavorite ? t('saved_fav') : t('save_fav')}</span></button>
              <button onClick={handleShare} className="py-4 rounded-2xl font-black uppercase tracking-widest text-[10px] bg-white/5 text-neutral-400 border border-white/10 hover:bg-white/10 hover:text-white transition-all flex items-center justify-center space-x-2"><FiShare2 size={18} /><span>{t('share')}</span></button>
            </div>
            ...
            <div className={`p-8 rounded-[2rem] border transition-colors ${theme === 'dark' ? 'bg-white/5 border-white/10 text-neutral-400' : 'bg-white border-gray-200 text-gray-600 shadow-lg'} space-y-4 text-xs font-bold uppercase tracking-widest`}>
              <div className="flex justify-between items-center"><FiStar className="text-yellow-500" /> <span className={`${theme === 'dark' ? 'text-white' : 'text-dark'} text-lg font-black`}>{movie.vote_average?.toFixed(1)}</span></div>
              <div className={`h-[1px] ${theme === 'dark' ? 'bg-white/5' : 'bg-gray-100'}`} />
              <div className="flex justify-between items-center">{t('views')}: <span className={theme === 'dark' ? 'text-white' : 'text-dark'}>{(movie.popularity * 10).toLocaleString()}</span></div>
            </div>
          </div>
              ...
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 relative z-10 border-b border-white/5 pb-8">
                ...
                <div className="flex items-center space-x-3 bg-black/20 backdrop-blur-md px-4 py-2 rounded-xl border border-white/5">
                  <div className="flex space-x-0.5">
                    {[1,2,3,4,5].map(s => (
                      <FiStar key={s} size={14} className={movie.vote_average/2 >= s ? 'text-yellow-500 fill-current' : 'text-neutral-800'} />
                    ))}
                  </div>
                  <span className="font-black text-lg text-yellow-500">{movie.vote_average?.toFixed(1)}</span>
                </div>
              </div>
                        ...
                        <div className="flex items-center justify-between pt-5 border-t border-white/5 mt-auto">
                          <div className="flex items-center gap-8">
                            <button 
                              ...
                              className={`text-[11px] font-black uppercase tracking-widest transition-all flex items-center space-x-2.5 ${c.likes?.includes(user?._id) ? 'text-primary scale-110' : 'text-neutral-500 hover:text-neutral-300'}`}
                            >
                            ...
                            {c.replies.map((reply, rid) => (
                              <div key={rid} className="flex gap-2 group/reply">
                                ...
                                <div className="flex-grow min-w-0">
                                  <div className="flex items-center justify-between gap-2">
                                    ...
                                    <div className="flex items-center space-x-2">
                                      ...
                                      <button 
                                        ...
                                        className={`text-[7px] font-black uppercase transition-all flex items-center space-x-1 ${reply.likes?.includes(user?._id) ? 'text-primary' : 'text-neutral-500 hover:text-neutral-300'}`}
                                      >
                                  ...
                                  <p className={`text-[10px] leading-snug font-medium break-words ${theme === 'dark' ? 'text-neutral-400' : 'text-gray-600'}`}>
                                    {reply.replyToUser && (
