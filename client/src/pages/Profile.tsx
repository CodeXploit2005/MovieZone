...
const Profile = () => {
  const { user, updateProfile, uploadAvatar, requestAdmin, logout, theme, t, lang, formatMsg } = useAuth();
  const navigate = useNavigate();
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [stats, setStats] = useState({
    favorites: 0,
    comments: 0,
    avgRating: 0 as string | number
  });
  ...
  // Sync formData with user state when user state changes (e.g. after avatar upload)
  useEffect(() => {
    if (user) {
      setFormData(prev => ({
        ...prev,
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        toast.error(result.message || t('error_update'));
      }
    } catch (error) {
  ...
  const handleLogout = () => {
    logout();
    navigate('/login');
    toast.info(formatMsg(t('logout_success')));
  };
            ...
            >
              ...
              <div className="space-y-4">
                ...
                <button 
                  ...
                  className={`w-full py-4 rounded-2xl font-black uppercase tracking-widest text-xs transition-all flex items-center justify-center space-x-3 border ${theme === 'dark' ? 'bg-white/5 border-white/10 text-neutral-400 hover:text-white' : 'bg-gray-100 border-gray-200 text-gray-500 hover:text-dark'}`}
                >
                  <FiLogOut />
                  <span>{t('logout')}</span>
                </button>
                  ...
                  >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      {[
                        ...
                        <div key={i} className={`p-8 rounded-[2rem] border text-center transition-all hover:-translate-y-2 ${theme === 'dark' ? 'bg-white/5 border-white/5' : 'bg-white border-gray-100 shadow-lg'}`}>
                    ...
                    <div className={`p-10 rounded-[2.5rem] border ${theme === 'dark' ? 'bg-white/5 border-white/10' : 'bg-white border-gray-100 shadow-xl'}`}>
                      <h3 className="text-xl font-black uppercase tracking-widest mb-8 flex items-center">
                      ...
                      <div className="space-y-2">
                        {[
                          ...
                          // Only show Request Admin if user is not admin
                          ...(user?.role !== 'admin' ? [{
